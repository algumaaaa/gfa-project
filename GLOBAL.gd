...
func _spawn_horde(value):
	...
	for s in n.get_children():
		...
		if disto < 500:
			var w = s.get_world().direct_space_state
			var r = w.intersect_ray(s.global_transform.origin, player.global_transform.origin, [player])
			if r:
				s._spawn_zombie(floor(value))
				#s._spawn_worm(floor(value * 0.5))
